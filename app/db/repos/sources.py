...
def list_all() -> list[Source]:
    with eng.session() as s:
        return list(s.exec(select(Source).order_by(col(Source.id))).all())
...
def claim_due(limit: int) -> list[Source]:
    with eng.session() as s:
        rows = s.exec(
            select(Source)
            .where(Source.status == "active", Source.next_scan_at <= utcnow())
            .order_by(col(Source.next_scan_at))
            .limit(limit)
...
def mark_success(source_id: int) -> None:
    with eng.session() as s:
        ...
        src.fail_count = 0
        src.last_error = ""
        src.next_scan_at = utcnow() + timedelta(seconds=config.SCAN_INTERVAL_SECONDS)
        s.add(src)
        s.commit()
...
def mark_failed(source_id: int, error: str) -> None:
    with eng.session() as s:
        ...
        src.next_scan_at = utcnow() + timedelta(seconds=backoff)
